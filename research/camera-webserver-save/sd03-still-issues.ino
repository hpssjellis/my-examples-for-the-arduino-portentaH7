...
//adapted for the Arduino Portenta By Jeremy Ellis
//
...
int32_t myRead(PNGFILE *handle, uint8_t *buffer, int32_t length) {
  ...
  //return fread(buffer, length, myfile);
  return fread(buffer, length, length, f);
}
int32_t myWrite(PNGFILE *handle, uint8_t *buffer, int32_t length) {
  File *f = (File *)handle->fHandle;
  ///return f->write(buffer, length);
  //return fprintf(buffer, length, myfile);
...
int32_t mySeek(PNGFILE *handle, int32_t position) {
  ...
  //return fseek(position, myfile);
  return fseek(position, f);
}
#endif // WRITE_TO_SD
