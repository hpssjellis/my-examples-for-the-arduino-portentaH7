...
// this works in the browser.  Note: Very very small red dot!
   ...
   */
   ...
   if (mode == 1) {    // show the image
        ...
        for (k = 0; k <= iDataSize; k++){
          ...
          //myImage += ucOut[k];
         // long hexVal = ltoa(ucOut[k], buf, 16);
         // myNew = ltoa(ucOut[k], buf, 16);

          
          myNew = String(ucOut[k], HEX);  // kind of works
         // myNew = String(hexVal);
          
          // some are these are only 1 digit so add a zero to it
          if (myNew.length() <= 1) {myNew += '0';}
          
          myNew.toUpperCase();
          myImage += myNew;
          //myImage += ",";   // put in a comma to see if 2 digits per hex
