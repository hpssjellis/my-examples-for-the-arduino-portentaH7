...
LoRaModem modem;
CayenneLPP lpp(51);   // 51 default length can make it smaller or larger but lean towards smaller
...
// Note: Best to have the Device_EUI hard coded. Run the program once to see the fixed Device_EUI value.
...
void setup() {
  ...
  // change this to your regional band (eg. EU868, US915, CN779, EU433, AU915, CN470, AS923, KR920, IN865, RU864 ...)
  ...
  Serial.print("Your Device_EUI is: ");
...
void loop() {
  ...
  if (millis() - myStoredMillisA >= myTimerDurationA){  // Test whether the period has elapsed
    myStoredMillisA = millis();                         // IMPORTANT to save the next stored time 
      ...
      int x = analogRead(A0)             // even if disconnected will get a reading around 100 can be 0-->1023;
      lpp.addGenericSensor(1, x);
    
      
      /*
      // Can do any of these CAYENNELPP set types.
      lpp.reset();
