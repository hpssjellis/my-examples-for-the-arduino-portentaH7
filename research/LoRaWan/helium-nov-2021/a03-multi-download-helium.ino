...
int myDelay= 60000;   // non-block delay in milliseconds
int myCheckReply= 1000;   // non-block delay in milliseconds
...
#define SECRET_APP_KEY "1FE8289451660F15C95798AD8CB688A2"
...
// skipping all the cayenneLPP just send raw data
  
// String msg = "Hello Lora\n";

//{"variable":"temperature","value":71}
//String msg = "[{\"variable\":\"temperature\",\"value\":71}]";
String msg = "Hi3";
  ...
  if (err > 0) { // why is an error greater than zero a good thing?
  ...
  delay(3000);   // I don't like delays but this is OK here
  if (!modem.available()) {
    Serial.println("No downlink message received at this time.");
    return;  // bumps us out of here
  }
  char rcv[64];
  int i = 0;
  ...
  Serial.println(String(rcv));
  Serial.print("Received as data: ");
...
// so lets always check for a downlink

if ( (millis() - checkStart ) >= myCheckReply) {  
    checkStart = millis(); //  reset the delay time 
 if (modem.available()) {
