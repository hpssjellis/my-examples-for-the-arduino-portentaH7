...
byte myCroppedData[96 * 96 * 2];  // cropped data
...
void loop() {
  ...
  myCrop(160, 120, myCroppedData);

 // Serial.write(data, bytesPerFrame); // test with myCroppedData
  Serial.write(myCroppedData, bytesPerFrame); // test with myCroppedData
}


void myCrop(int image_width, int image_height, uint8_t* image_data) {

      //int min_x = (176-96)/2;
      //int min_y = (144-96)/2;
      //int min_x = (160-96)/2;
      int min_x = (160-96)/2;   // for 2 bytes
      int min_y = (120-96)/2;
      int index = 0;

      // Crop 96x96 image
      for (int y = min_y; y < min_y+96; y++) {
        for (int x = min_x; x < (min_x+96)*2; x++) {  // for 2 bytes
         // image_data[index++] = data[(y*176)+x];
          image_data[index++] = data[(y*160)+x];
        }
      }
